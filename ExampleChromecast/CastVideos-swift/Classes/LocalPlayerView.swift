...
class LocalPlayerView: UIView {
  ...
  fileprivate var youboraWrapper: YBAVPlayerAdapterSwiftWrapper?
  fileprivate var youboraAdapter: YBAVPlayerAdapter?
  fileprivate var youboraPlugin: YBPlugin?
      ...
      if let contentID = media?.contentID {
        if let mediaURL = URL(string: contentID) {
          ...
          addMediaPlayerObservers()
            ...
            if youboraWrapper == nil {
                YBLog.setDebugLevel(.verbose)
                let options = YBOptions()
                options.accountCode = "nicetest"
                youboraPlugin = YBPlugin(options: options)
                youboraAdapter = YBAVPlayerAdapter(player: mediaPlayer!)
                youboraWrapper = YBAVPlayerAdapterSwiftWrapper(adapter: youboraAdapter, andPlugin: youboraPlugin)
            }
        }
      }
    }
